...
var $author$project$Main$busyBeaver = {
	currentState: 'A',
...
var $author$project$App$ComputationWorkflow$Impl$init = {id: 0, step: $elm$core$Maybe$Nothing};
...
var $author$project$Main$init = function (turing) {
	return _Utils_Tuple2(
		{
			activeComputationWorkflow: $author$project$App$ComputationWorkflow$Impl$init,
...
var $author$project$Core$Turing$applyRule = F2(
	function (_v0, turing) {
		...
		var newSymbol = _v0.newSymbol;
		var newState = _v0.newState;
		var moveDirection = _v0.moveDirection;
		return ((!_Utils_eq(turing.currentState, currentState)) || (!_Utils_eq(turing.tape.currentSymbol, currentSymbol))) ? $elm$core$Maybe$Nothing : $elm$core$Maybe$Just(
			_Utils_update(
				turing,
				{
		...
		function (rules, index) {
			...
			while (true) {
				if (rules.b) {
					...
					var currentSymbol = r.currentSymbol;
					var rs = rules.b;
					if (_Utils_eq(turing.currentState, currentState) && _Utils_eq(turing.tape.currentSymbol, currentSymbol)) {
						return $elm$core$Maybe$Just(
							_Utils_Tuple2(index, r));
					} else {
						var $temp$rules = rs,
							$temp$index = index + 1;
						rules = $temp$rules;
...
var $author$project$Core$Tape$fromString = F3(
	function (parseSymbol, emptySymbol, string) {
		var unwrapSecond = function (_v4) {
			var a = _v4.a;
			var mb = _v4.b;
			return A3(
		...
		var parseOptionalBrackets = function (s) {
			var isBracketed = A2($elm$core$String$startsWith, '[', s) && A2($elm$core$String$endsWith, ']', s);
			var remainingChars = isBracketed ? A3($elm$core$String$slice, 1, -1, s) : s;
			return _Utils_Tuple2(isBracketed, remainingChars);
		};
		var maybeSymbols = $elm_community$maybe_extra$Maybe$Extra$combine(
			A2(
				$elm$core$List$map,
				function (s) {
					return unwrapSecond(
		...
		return A2(
			...
			function (_v0) {
				var symbols = _v0.a;
				var focusedSymbolIndex = _v0.b;
				var _v1 = A2($elm_community$list_extra$List$Extra$splitAt, focusedSymbolIndex, symbols);
				if (_v1.b.b) {
					var left = _v1.a;
					var _v2 = _v1.b;
					var _v3 = _v2.a;
					...
					return $elm$core$Result$Err('Unexpected split error');
				}
			},
			A2(
				$elm$core$Result$andThen,
				function (symbols) {
					return A2(
						$elm$core$Result$map,
						function (index) {
							return _Utils_Tuple2(symbols, index);
						},
						A2(
							$elm$core$Result$fromMaybe,
							'Initial symbol not specified',
							A2($elm_community$list_extra$List$Extra$findIndex, $elm$core$Tuple$first, symbols)));
				},
...
var $author$project$App$ComputationWorkflow$Impl$reset = function (workflow) {
	return {id: workflow.id + 1, step: $elm$core$Maybe$Nothing};
};
var $author$project$App$ComputationWorkflow$Step$ComputeNextState = {$: 'ComputeNextState'};
...
var $author$project$App$ComputationWorkflow$Impl$start = function (workflow) {
	return A2(
		$elm$core$Task$perform,
		function (_v0) {
				...
				_Utils_update(
					workflow,
					{
						step: $elm$core$Maybe$Just(
							A2($elm$core$Maybe$withDefault, $author$project$App$ComputationWorkflow$Step$ComputeNextState, workflow.step))
					}));
		},
		$elm$time$Time$now);
};
var $author$project$Main$restartComputationIfRunning = function (model) {
	var newComputationWorkflow = model.isRunning ? $author$project$App$ComputationWorkflow$Impl$reset(model.activeComputationWorkflow) : model.activeComputationWorkflow;
...
var $author$project$App$ComputationWorkflow$Step$NewSymbolFadein = {$: 'NewSymbolFadein'};
							...
							A2(
								...
								$author$project$App$Msg$ProcessComputationWorkflow(
									_Utils_update(
										workflow,
										{
											step: $elm$core$Maybe$Just($author$project$App$ComputationWorkflow$Step$NewSymbolFadein)
										}))));
							...
							A2(
								...
								$author$project$App$Msg$ProcessComputationWorkflow(
									_Utils_update(
										workflow,
										{
											step: $elm$core$Maybe$Just($author$project$App$ComputationWorkflow$Step$UpdateMachineState)
										}))));
					default:
						var _v7 = _v0.a;
						...
						var cmd = $author$project$Core$Turing$isHalted(newTuring) ? A2($andrewMacmurray$elm_delay$Delay$after, 0, $author$project$App$Msg$ToggleComputation) : A2(
							...
							$author$project$App$Msg$ProcessComputationWorkflow(
								_Utils_update(
									workflow,
									{
										step: $elm$core$Maybe$Just($author$project$App$ComputationWorkflow$Step$ComputeNextState)
									})));
						return _Utils_Tuple2(
...
var $author$project$App$Model$validateTapeString = function (tapeString) {
	return $elm$core$String$isEmpty(tapeString) ? $elm$core$Maybe$Just('Tape unspecified') : $elm$core$Maybe$Nothing;
};
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'UpdateState':
				...
				var validationError = $author$project$App$Model$validateStateString(sanitizedValue);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
					...
					$elm$core$Platform$Cmd$none);
			case 'UpdateEmptySymbol':
				var newValue = msg.a;
				var sanitizedValue = $elm$core$String$trim(newValue);
				var validationError = $author$project$App$Model$validateEmptySymbolString(sanitizedValue);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							...
							currentTapeValidationError: A2(
								$elm_community$maybe_extra$Maybe$Extra$or,
								validationError,
								$elm_community$result_extra$Result$Extra$error(newTape)),
							turing: A2(
								$author$project$Core$Turing$asTapeIn,
								model.turing,
			...
			case 'ToggleComputation':
				var newComputationWorkflow = model.isRunning ? $author$project$App$ComputationWorkflow$Impl$reset(model.activeComputationWorkflow) : model.activeComputationWorkflow;
				var cmd = model.isRunning ? $elm$core$Platform$Cmd$none : $author$project$App$ComputationWorkflow$Impl$start(newComputationWorkflow);
				return _Utils_Tuple2(
					_Utils_update(
						model,
			...
			case 'StepFw':
				...
				if (_v5.$ === 'Just') {
					...
					var _v7 = $author$project$Main$restartComputationIfRunning(model);
			...
			default:
				var _v8 = $author$project$Main$restartComputationIfRunning(model);
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$rtfeldman$elm_css$Html$Styled$text(toggleBtnText)
							])),
						A2(
						$rtfeldman$elm_css$Html$Styled$div,
						_List_fromArray(
...
var $author$project$Main$editStateAndTapeHtml = function (model) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
			[
...
var $author$project$Main$rulesListEntryHtml = F3(
...
var $author$project$Main$rulesListHtml = function (model) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
			[
				$rtfeldman$elm_css$Html$Styled$Attributes$class('rules-container')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$Attributes$class('rules-list')
					]),
				A2(
					$elm$core$List$indexedMap,
					$author$project$Main$rulesListEntryHtml(model),
					model.ruleStrings)),
				A2(
				$rtfeldman$elm_css$Html$Styled$div,
				_List_fromArray(
...
var $author$project$Main$stateAndTapeHtml = function (model) {
	...
	return A2(
		...
		_List_fromArray(
			[
				$rtfeldman$elm_css$Html$Styled$Attributes$class('state-and-tape-container')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text(renderedState)
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$div,
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$Attributes$class('tape-wrapper')
					]),
				_List_fromArray(
					[
						A2(
...
var $author$project$Main$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$author$project$Main$stateAndTapeHtml(model),
				$author$project$Main$editStateAndTapeHtml(model),
				$author$project$Main$controlsHtml(model),
				$author$project$Main$rulesListHtml(model)
			]));
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{
		init: function (_v0) {
			return $author$project$Main$init($author$project$Main$busyBeaver);
		},
